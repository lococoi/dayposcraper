...
class ScraperApp(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(input_frame, text="URL del Test:").grid(
            row=0, column=0, padx=10, pady=10, sticky="w"
        )
        self.url_entry = ctk.CTkEntry(
            input_frame,
            placeholder_text="Ej: https://www.daypo.com/examen-intelectual-isep-santa-fe.html#test",
        )
        self.url_entry.grid(row=0, column=1, padx=(0, 10), pady=10, sticky="ew")

        ctk.CTkLabel(input_frame, text="Archivo JSON:").grid(
            row=1, column=0, padx=10, pady=10, sticky="w"
        )
        self.file_entry = ctk.CTkEntry(
            input_frame, placeholder_text="Ej: resultados.json"
        )
        ...
        self.start_button = ctk.CTkButton(
            control_frame, text="Scrapear", command=self.start_scraping_thread
        )
        self.start_button.grid(row=0, column=0, padx=10, pady=10, sticky="ew")

        self.stop_button = ctk.CTkButton(
            control_frame, text="Detener", command=self.stop_scraping, state="disabled"
        )
        ...
        ctk.CTkLabel(self, text="Registro").grid(
            row=3, column=0, padx=10, pady=(5, 0), sticky="sw"
        )
    ...
    def _run_scraper_logic(self):
        ...
        try:
            ...
            try:
                ...
                self.log(
                    "ERROR: No se pudo obtener el número total de preguntas (ID 'cuestion'). Abortando."
                )
                return
            ...
            while self.is_running:
                ...
                if "error" in datos_pregunta or datos_pregunta.get("pregunta") == "N/D":
                    self.log(
                        f"Fin o Error: {datos_pregunta.get('detalle', 'Fin del test.')}"
                    )
                    break
                ...
                if pregunta_id not in [
                    r.get("id_pregunta") for r in resultados if isinstance(r, dict)
                ]:
                    pregunta_formateada = {
                    ...
                    self.log(
                        f"Pregunta {len(resultados)}/{self.total_questions}: {datos_pregunta['pregunta']}.\nRespuesta: {pregunta_formateada['respuesta'][0]}\n"
                    )
                    ...
                    resultados_final = [
                        {
                            ...
                            if k not in ["id_pregunta", "cadena_marcadores"]
                        }
            ...
            self.log(
                f"ERROR: Fallo de Selenium. Verifique el driver y la URL. Detalle: {e}"
            )
